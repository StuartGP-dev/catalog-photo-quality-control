...
COARSE_VALUES = {
    "rotation": (0.25, 0.5, 0.8, 1.2, 2.0, 3.5, 5.0, 6.5, 8.0),
    "crop": (0.003, 0.006, 0.01, 0.02, 0.04, 0.06, 0.08, 0.10, 0.12),
    "zoom": (1.003, 1.006, 1.01, 1.02, 1.04, 1.07, 1.10, 1.15, 1.20),
    "dezoom": (0.995, 0.99, 0.98, 0.96, 0.93, 0.90, 0.85, 0.80),
    "offset": (0.003, 0.006, 0.01, 0.02, 0.04, 0.06, 0.08, 0.10, 0.12),
}
FAMILY_THRESHOLDS = {
...
def _write_report(path: Path, listing: SourceListing, config_hash: str, results: Sequence[CalibrationResult]) -> None:
    ...
    for family in sorted({result.spec.family for result in results}):
        ...
        for number, result in enumerate(family_results):
            ...
            for source, output, metrics in zip(listing.images, result.rendered.images, result.per_image, strict=True):
                ...
                <div class="side-by-side"><figure><figcaption>Originale</figcaption><img src="{original}" alt="image originale complète"></figure><figure><figcaption>Photo filtrée</figcaption><img src="{variant}" alt="photo filtrée complète"></figure></div>
    ...
    <style>body{{font:14px system-ui;margin:1.5rem;background:#f3f4f6;color:#1f2937}}main{{max-width:1600px;margin:auto}}section,.example,.image-card,.summary,.review-toolbar{{background:#fff;padding:1rem;margin:1rem 0;border-radius:10px;box-shadow:0 1px 5px #0002}}.review-toolbar{{position:sticky;top:0;z-index:20;display:flex;gap:.6rem;align-items:center;flex-wrap:wrap}}.example{{border-left:7px solid #94a3b8}}.example.human-accepted{{outline:4px solid #16a34a}}.example.human-rejected{{outline:4px solid #dc2626}}.example.human-unsure{{outline:4px solid #eab308}}.perceptible_candidate{{border-color:#16a34a}}.strong_candidate{{border-color:#eab308}}.rejected{{border-color:#dc2626}}.review{{display:flex;gap:.5rem;align-items:center;flex-wrap:wrap;padding:.7rem;background:#eef2ff}}.review-note{{min-width:280px;flex:1}}.side-by-side{{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:1rem}}.side-by-side figure{{margin:0;background:#e5e7eb;padding:.6rem;border-radius:8px}}.side-by-side figcaption{{font-size:1.1rem;font-weight:800;margin-bottom:.5rem;text-align:center}}.side-by-side img{{display:block;width:100%;height:520px;object-fit:contain;background:#fff}}.tools{{display:grid;grid-template-columns:repeat(auto-fit,minmax(210px,1fr));gap:.5rem}}figure{{margin:0}}.tools img{{width:100%;height:190px;object-fit:contain;background:#eee}}pre{{max-height:22rem;overflow:auto}}code{{overflow-wrap:anywhere}}.badge{{font-weight:700}}#save-status{{font-weight:700}}@media(max-width:900px){{.side-by-side{{grid-template-columns:1fr}}}}</style></head><body><main>
    <h1>Calibration visuelle géométrique</h1><div class="summary"><p>Annonce {html.escape(listing.listing_code)} · {len(listing.images)} images · source_set_hash <code>{listing.source_set_hash}</code></p><p>Hash de configuration <code>{config_hash}</code> · version {CALIBRATION_VERSION}</p><p>{html.escape(str(counts))}</p><p>Chaque originale est affichée à gauche et sa variante complète à droite. Les métriques sont informatives.</p></div><div class="review-toolbar"><strong id="review-counts">0 évalué</strong><select id="review-filter"><option value="all">Toutes</option><option value="pending">Non évaluées</option><option value="accepted">Acceptées</option><option value="rejected">Refusées</option><option value="unsure">À revoir</option></select><button id="export-reviews" type="button">Sauvegarder mes choix</button><label>Importer <input id="import-reviews" type="file" accept="application/json"></label></div>{''.join(sections)}
    <script>const META={json.dumps({'version': CALIBRATION_VERSION, 'listing': listing.listing_code, 'source_set_hash': listing.source_set_hash, 'config_hash': config_hash}, ensure_ascii=False)};const STORAGE='catalog-review:'+META.source_set_hash+':'+META.config_hash;let reviews=JSON.parse(localStorage.getItem(STORAGE)||'{{}}');const cards=[...document.querySelectorAll('.example')];function paint(card){{const r=reviews[card.dataset.reviewKey];card.classList.remove('human-accepted','human-rejected','human-unsure');if(r){{card.classList.add('human-'+r.decision);card.querySelector('.review-state').textContent=r.decision;card.querySelector('.review-note').value=r.note||''}}else card.querySelector('.review-state').textContent='Non évalué'}}function save(card,decision){{reviews[card.dataset.reviewKey]={{decision,note:card.querySelector('.review-note').value,family:card.dataset.family,parameters:JSON.parse(card.dataset.parameters)}};localStorage.setItem(STORAGE,JSON.stringify(reviews));paint(card);update()}}function update(){{const values=Object.values(reviews);document.querySelector('#review-counts').textContent=`${{values.length}}/${{cards.length}} évalués · ${{values.filter(x=>x.decision==='accepted').length}} acceptés · ${{values.filter(x=>x.decision==='rejected').length}} refusés`;const filter=document.querySelector('#review-filter').value;cards.forEach(c=>{{const d=reviews[c.dataset.reviewKey]?.decision;c.hidden=filter!=='all'&&(filter==='pending'?!!d:d!==filter)}})}}cards.forEach(c=>{{c.querySelectorAll('[data-decision]').forEach(b=>b.onclick=()=>save(c,b.dataset.decision));c.querySelector('.review-note').onchange=()=>{{if(reviews[c.dataset.reviewKey])save(c,reviews[c.dataset.reviewKey].decision)}};paint(c)}});document.querySelector('#review-filter').onchange=update;document.querySelector('#export-reviews').onclick=()=>{{const payload={{...META,exported_at:new Date().toISOString(),reviews}};const a=document.createElement('a');a.href=URL.createObjectURL(new Blob([JSON.stringify(payload,null,2)],{{type:'application/json'}}));a.download=`human_filter_choices_${{META.listing}}.json`;a.click();URL.revokeObjectURL(a.href)}};document.querySelector('#import-reviews').onchange=async e=>{{const payload=JSON.parse(await e.target.files[0].text());if(payload.source_set_hash!==META.source_set_hash||payload.config_hash!==META.config_hash){{alert('Fichier incompatible avec cette campagne');return}}reviews=payload.reviews||{{}};localStorage.setItem(STORAGE,JSON.stringify(reviews));cards.forEach(paint);update()}};document.querySelectorAll('.compare').forEach(box=>{{let timer=null;const overlay=box.querySelector('.overlay');const slider=box.parentElement.querySelector('.slider');slider.addEventListener('input',()=>overlay.style.clipPath=`inset(0 0 0 ${{slider.value}}%)`);box.parentElement.querySelector('.toggle').onclick=()=>{{overlay.style.display=overlay.style.display==='none'?'block':'none'}};box.parentElement.querySelector('.blink').onclick=e=>{{if(timer){{clearInterval(timer);timer=null;e.target.textContent='Alternance'}}else{{timer=setInterval(()=>overlay.style.display=overlay.style.display==='none'?'block':'none',350);e.target.textContent='Arrêter'}}}};box.parentElement.querySelector('.zoom').onclick=()=>box.classList.toggle('actual')}});update();</script></main></body></html>'''
